...
def add_square_grid(board: pcbnew.BOARD) -> None:
    ...
    for x, y in square_grid_dot_positions(dimensions):
        dot = pcbnew.PCB_SHAPE(board)
        dot.SetShape(pcbnew.SHAPE_T_SEGMENT)
        dot.SetStart(native.point(x - half_segment, y))
        dot.SetEnd(native.point(x + half_segment, y))
...
def add_front_silkscreen(board: pcbnew.BOARD) -> None:
    ...
    _add_text(
        board,
        f"CHESS BOARD {board.GetTitleBlock().GetRevision()}",
        (116.0, -165.0),
        height=1.5,
    )
    ...
    for bank, (ref, _) in zip(
        ...
        at = (
            pcbnew.ToMM(module.GetPosition().x) - native.ORIGIN_X_MM,
            native.ORIGIN_Y_MM
            - pcbnew.ToMM(min(courtyard_y))
            + TCA9554_SILKSCREEN_CLEARANCE_MM,
        )
        _add_text(
            board, f"{ref}  I2C 0x{bank.address:02X}  {bank.label}", at, height=0.8
        )
