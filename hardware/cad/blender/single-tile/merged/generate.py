...
import tile_electronics  # noqa: E402


TOP_PATH = PROJECT_DIR.parent / "top" / "tile-top.blend"
BOTTOM_PATH = PROJECT_DIR.parent / "bottom" / "tile-bottom.blend"
OUTPUT_PATH = PROJECT_DIR / "single-tile-merged.blend"
...
def render_views(
    ...
    scene.render.filepath = str(RENDER_DIR / "merged.png")
    ...
    camera.data.lens = 58
    modeling.point_at(camera, Vector((0.0, 0.0, 3.0)))
    scene.render.filepath = str(RENDER_DIR / "open.png")
    bpy.ops.render.render(write_still=True)
    ...
    scene.render.filepath = str(RENDER_DIR / "wired.png")
    bpy.ops.render.render(write_still=True)
    top.location = top_location
    top.rotation_euler = top_rotation
...
def build() -> None:
    ...
    scene["top_source"] = str(TOP_PATH.relative_to(BLENDER_ROOT))
    scene["bottom_source"] = str(BOTTOM_PATH.relative_to(BLENDER_ROOT))
